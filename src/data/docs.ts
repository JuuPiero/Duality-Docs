...
export const docPages: DocPage[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'heading', text: 'Baked vertex color and renderer diagnostics / Vertex color bake và chẩn đoán renderer' },
      { type: 'paragraph', text: 'MeshVertex now carries Color in addition to position, UV and normal. An OBJ line may use v x y z r g b [a]; normalized 0–1 values and byte-range 0–255 values are accepted. The color multiplies Material.Color, texture and VertexLit output. This is the current baked-lighting path: bake AO/static light in a DCC tool, export it as vertex color, then render it with no extra texture sample, render target or lighting pass. Primitive meshes and older OBJ files use white, so existing visuals remain unchanged.' },
      { type: 'callout', tone: 'success', title: '3DS-friendly static lighting', text: 'Vertex colors are interpolated by the GPU as part of the existing mesh stream. They work for Unlit and VertexLit meshes; SpriteRenderer and Canvas UI intentionally remain unlit and do not use this path.' },
      { type: 'paragraph', text: 'The Game panel reports exact submitted mesh draw calls, visible/cull mesh counts, VertexLit draw calls, projected blob-shadow draws and visible sprites. Mesh culling uses a conservative world-space bounding sphere against the active camera frustum. It only rejects meshes safely outside the view; it never changes 2D sprite or UI composition.' }
    ]
  },
  {
  ...
  },
  {
    ...
    blocks: [
      { type: 'table', headers: ['Area', 'Core behaviour'], rows: [
        ...
        ['Content Browser', 'Resizable tree at left shows assets/packages and expandable imported-model children; grid at right supports multi-select, drag/drop, Ctrl+D and direct scene drops.'],
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'paragraph', text: 'A DirectionalLightComponent direction is derived from the entity world rotation. SceneRenderer, not scripts or a backend, selects the first active enabled directional light in canonical scene-hierarchy order for a camera RenderView. This makes main-light selection reproducible even when EnTT component-storage order changes. The editor’s free Scene camera calls that same selection helper, so its 3D preview matches Game lighting without modifying a gameplay camera. Materials stay reusable assets. The implemented first MaterialPropertyBlock slice is a runtime-only per-MeshRenderer Color override: it is not reflected or serialized, changes neither the .mat nor other users of it, and leaves Texture and ShadingMode authored by the material.' },
      ...
      { type: 'list', items: ['Material loader regression: VertexLit serialises and deserialises; missing Shading Mode remains Unlit.', 'MeshRenderer runtime-material regression: a scripting Color override reaches SceneRenderer, remains isolated to that renderer, and is excluded from scene serialization.', 'Mesh loader regression: authored OBJ vn normals and vertex colors survive import; a position/UV-only OBJ gets a non-zero generated triangle normal; each built-in primitive exposes non-zero normalized normals.', 'Scene serializer regression: DirectionalLightComponent colour, intensity and CastShadows field round-trip.', 'RenderView regression: camera state is preserved, disabled directional lights are skipped, and the selected main-light direction/parameters are deterministic by hierarchy order.', 'Blob-shadow regression: a cube above one Plane receiver emits one transparent depth-read-only Plane command at the receiver. Frustum-culling regression verifies one accepted and one rejected mesh plus submission diagnostics.', 'Run the desktop suite through run-tests.bat so the devkitPro runtime DLL path is present. The current suite has 78 passing tests.', 'Picasso regression: build the mesh_shader target in the 3DS build tree; the PICA VertexLit shader must compile before a device package is produced.', 'Before declaring VertexLit supported on device, run a Citra and real Old 3DS scene with both screens active, a textured mesh, an untextured mesh, no light, a rotated light and non-uniform scale.'] },
      { type: 'callout', tone: 'warning', title: 'Current shadow boundary', text: 'Blob Shadows remain the hardware-safe 3DS tier: non-plane meshes project onto active Plane primitives after opaque rendering, with alpha blending and depth writes disabled. Desktop additionally has an experimental 512×512 directional depth-map pass. It is camera-following and intentionally bounded; it is not a promise of cascades, arbitrary receiver filtering, light cookies or production-quality PC shadows.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'code', language: 'cpp', code: 'enum class MaterialShadingMode { Unlit, VertexLit };\nenum class ShadowMode { Off, BlobShadows, ShadowMapsExperimental };\n\nstruct DirectionalLightComponent {\n    bool Enabled = true;\n    glm::vec3 Color{ 1.0f };\n    float Intensity = 1.0f;\n    bool CastShadows = false; // Per-light opt-in.\n};' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'heading', text: 'Implemented Build Settings policy / Chính sách Build Settings đã triển khai' },
      { type: 'paragraph', text: 'Build Settings > 3D Shadows now persists ShadowTechnique in the .dproj and bakes the same value into romfs/BuildSettings.json for a 3DS build. Editor Scene/Game views, DualityPlayerDesktop and DualityPlayer apply the same RenderSettings policy. Cấu hình không phụ thuộc vào Anti-Aliasing: thay đổi AA không tự bật/tắt shadow.' },
      { type: 'table', headers: ['Setting', 'Current behaviour', '3DS guidance'], rows: [
        ['Off', 'No projected shadow draw calls. VertexLit directional shading remains active.', 'Use for UI-heavy, sprite-heavy or maximum-performance scenes.'],
        ['Blob Shadows (Recommended)', 'Directional Light with Cast Shadows projects a translucent, depth-read-only quad from non-Plane meshes to active Plane receivers.', 'Supported default for Old/New 3DS; no depth texture and no second camera pass.'],
        ['Shadow Maps (Experimental)', 'OpenGL renders a 512×512 directional depth map then samples it during the mesh pass. Citro3D declines the optional target and SceneRenderer falls back to Blob.', 'Desktop preview/experimentation only. 3DS stays safe: no unbudgeted shadow texture or second pass is allocated.']
      ] },
      { type: 'code', language: 'cpp', code: '// ProjectConfig, stored as integer in .dproj / BuildSettings.json\nShadowMode ShadowTechnique = ShadowMode::BlobShadows;\n\n// Renderer policy, normally set by the host -- gameplay should not mutate it.\nRenderSettings::SetShadowMode(ShadowMode::BlobShadows);\n\n// A light must still opt in independently:\nauto& light = sun.AddComponent<DirectionalLightComponent>();\nlight.CastShadows = true;' },
      { type: 'list', items: ['Both gates are intentional: Shadow Technique chooses the project performance tier, and Directional Light > Cast Shadows chooses whether the selected main light can use that tier.', 'Blob casters are active MeshRenderer entities except Plane. Receivers are active MeshRenderer Plane primitives on the same screen/camera-visible layer. SpriteRenderer, Canvas and UI remain unlit and neither cast nor receive these shadows.', 'Desktop Shadow Maps submit visible mesh geometry to a camera-following orthographic light pass, then darken the main mesh pass with the resulting depth comparison. The pass is experimental because it has one compact coverage region and no cascades/PCF.', 'Only the first active enabled Directional Light in canonical hierarchy order becomes RenderView.MainLight. Keep one sun per scene when a predictable authoring result matters.', 'Old 3DS validation uses Blob Shadows, both displays active, expected maximum visible mesh count and real hardware profiling. Citro3D currently reports no shadow-target support, so it transparently falls back instead of consuming VRAM.'] },
      { type: 'callout', tone: 'warning', title: 'Shadow-map gate', text: 'Desktop Shadow Maps are now an experimental diagnostic tier, not the default quality setting. Do not promote them to 3DS until a fixed VRAM budget, real Old 3DS profiling, a PICA shadow texture/fragment-combiner implementation and failure-safe target allocation have all been verified.' }
    ]
  },
  {
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'table', headers: ['Layer', 'Owns', 'Must not own'], rows: [
        ...
        ['MaterialLoader / AssetDatabase', 'Material JSON and GUID-to-path resolution.', 'GPU residency or per-renderer runtime property state; a renderer loads the resolved path into its own cache.'],
      ...
      { type: 'paragraph', text: 'Skinned Mesh Renderer is the dedicated component for rigged assets. Assimp cooking writes up to four bone influences per vertex plus bone bind poses into .dmesh. It owns a Mesh, material slots and optional ordered scene bone EntityRefs; explicit entries match the imported mesh bone list, while empty entries fall back to a scene entity with the imported bone name. The component is intentionally distinct from Mesh Renderer so static meshes do not carry skinning state.' },
      { type: 'paragraph', text: 'MeshRendererComponent references an optional Mesh plus an optional Material list. At present the runtime MeshLoader consumes OBJ data; source-model children discovered from FBX/glTF/GLB become cooked .dmesh assets in the next import stage. An empty/unresolved Mesh falls back to MeshPrimitive. An imported mesh can contain submesh ranges; SceneRenderer resolves one material per range and emits one DrawMesh call for each range. One material entry covers all ranges; multiple entries are index-matched and unspecified ranges receive the default white unlit material.' },
      { type: 'list', items: ['Model source import: on desktop, Assimp inventories OBJ, FBX, glTF and GLB source models. Each Mesh child is immediately cooked to a private Assets/.duality-import/<source-guid>/*.dmesh asset with its own stable GUID; Animation children similarly become *.anim. Expand the parent arrow in Project tree or grid, then drag a Mesh child directly onto Mesh Renderer > Mesh or an Animation child onto Animation > Clip. The private folder is hidden from the browser but included by the build cooker.', 'The .dmesh reader is part of DualityEngine and has no Assimp dependency, so the same MeshRenderer reference works in desktop and cooked 3DS romfs. The 3DS player never parses FBX/glTF or links Assimp.', 'Skinned .dmesh V3 uses offline bone-palette partitioning: the cooker greedily clusters triangles whose retained bone sets fit a 24-bone local palette, then remaps each vertex to byte-sized local indices. It does not remove triangles or globally reduce a rig. On 3DS the palette consumes 72 of the 96 PICA float-vector uniforms as compact affine 3x4 matrices; camera and lighting use the rest. Larger rigs become several palette draw calls, which is a deliberate draw-call versus uniform-memory trade-off.', 'Pose matrices are cached by the skeleton world-transform signature, so Top, Bottom and editor camera views share one evaluated pose. Desktop GLSL and the PICA shader skin vertices on GPU; V1/V2 legacy cooked meshes retain a CPU dynamic-buffer fallback that updates only when that pose changes, never once per render view. New imports always write V3.', 'Animation is the initial transform-keyframe player: Enabled, Clip, Play On Awake, Loop, Speed and Playing are serialized; elapsed Time is runtime-only. It samples the imported channel matching the entity Name (or the sole channel of a one-node clip). Full rig hierarchy instantiation/skinning and blend trees are later work.', 'Material is a reusable .mat JSON asset with Color, Texture and Shading Mode. MaterialLoader returns a default material on missing/invalid data rather than failing a frame.', 'MeshRenderer::SetMaterialColor is the initial runtime MaterialPropertyBlock API. It replaces Color only for one renderer, preserves its assigned material texture/shading mode, is not serialized, and does not write the .mat or romfs. ClearMaterialColor removes it. A complete multi-property MaterialInstance remains later work.', 'AssetRef contains a GUID. AssetDatabase maps the GUID to a project path on desktop or a cooked romfs manifest path on device.', 'IRenderer2D::LoadTexture and IRenderer3D::LoadTexture maintain separate backend caches. Handle 0 means no texture; the draw uses flat color or a safe fallback.', 'IRenderer3D::LoadMesh uploads imported vertex data and returns a backend-local handle. Imported handles and built-in MeshPrimitive values use separate storage.', 'UnloadAllTextures and UnloadAllMeshes are scene-transition operations. They invalidate backend handles and must never run while an active render pass still references them.'] },
